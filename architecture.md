...
Throughout our project, we have made many architectural design decision which were set to help contribute to the project. Some of these decisions were to use different API's to support the functions we were adding. By implementing these API's (such as Leaflet.js) it has been easier to reach our clients requirements. Another decision we had was to use different languages to create the system. Some of these languages include Java, HTML, CSS and MySQL. The design decisions from Worksheet 2 provided us with guidelines as to how we should implement features into Worksheet 3. For example, we decided to use Leaflet.js to implement OpenStreetMaps and this provided to be beneficial for us in this worksheet. On the other hand, we had many design decisions which we had to change due to it clashing with other features of the system. This resulted in us having to find a different way to implement a feature into our system, resulting in a change to our backlog and risklog. We had to ensure that our service was fully secure and that it could not be bypassed. We did this by using different means of security, our main one being Spring Security.
